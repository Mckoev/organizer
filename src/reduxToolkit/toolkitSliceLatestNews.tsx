...
const toolkitSliceLatestNews = createSlice({
    name: 'latestNews',
    initialState: {
        arrNews: [init, init, init, init, init, init, init, init, init, init],
    },
    reducers: {
        newsAction(state, action) {
            for (let i = 0; i < action.payload.length; i++) {
                state.arrNews[i].creator = action.payload[i].creator
                ...
                state.arrNews[i].image_url = action.payload[i].image_url
